...
class Extractor:
    ...
    def process_tar_archive(self, archive_file):
        try:
            obj_tar_file = tarfile.open(self.source_path + "/" + archive_file)
    ...
    def process_rar_archive(self, archive_file):
        try:
            with rar_file.RarFile(self.source_path + "/" + archive_file) as obj_rar_file:
    ...
    def process_zip_archive(self, archive_file):
        try:
            with zipfile.ZipFile(self.source_path + "/" + archive_file, "r") as zip_file:
                print("Extracting - " + zip_file.filename)
                zip_file.extractall(self.destination_path + "/" + archive_file[:str(archive_file).find(".z")])
        except Exception as e:
